...
build: | $(O)  ## Build reflect binaries
	go build -o $(O) -ldflags='$(GO_LDFLAGS)' $(redirect)

install:  ## Build and install binaries in $GOBIN
	go install -ldflags='$(GO_LDFLAGS)' $(redirect)
...
release: ## Tag and release binaries for different OS on GitHub release
	...
	docker buildx build --push --build-arg=VERSION=$(VERSION) --tag julia/redirect:$(VERSION) --tag julia/redirect:latest .
